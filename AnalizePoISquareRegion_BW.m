function [ isElected ] = AnalizePoISquareRegion_BW( PoI, slope, squareSize, image, par )
    ...
    for i=1:4
        ...
        points(1,:) = squareSize * [ cos(slope+(i-1)*pi/2) , sin(slope+(i-1)*pi/2)] + PoI;
        points(2,:) = squareSize * sqrt(2) * [ cos(slope+(i-1)*pi/2+pi/4) , sin(slope+(i-1)*pi/2+pi/4)] + PoI; 
        points(3,:) = squareSize * [ cos(slope+(i-1)*pi/2+pi/2) , sin(slope+(i-1)*pi/2+pi/2)] + PoI; 
        points(4,:) = PoI;
        ...
        for j=1:area
           val1 = val1 + WhiteMagnitude(image(col(j), row(j),:),'bw');
           val2 = val2 + BlackMagnitude(image(col(j), row(j),:),'bw');
        end
...
%     whiteThreshold = 0.66;
    ...
    blackThreshold = par(1);
    whiteThreshold = par(2);
