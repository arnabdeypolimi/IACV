function [ imageSubpixData, houghMatrix, pool ] = SubpixelFilterMasked( bw_image, mask, filter_size, param )
% BW_IMAGE: black and white image to be analyzed;
% FILTER_SIZE: dimension of patches to be analyzed;
% MASK: mask from which select the points that have to be analyzed by the
% filter;
% FILTER_SIZE: dimension of the patch to analyze in order to estimate the
% local gradient of the pixel
% PARAM: contains the parameters to create the hough-like pool/matrix
    ...
    [globalYCoords, globalXCoords] = find(mask == 1);
  
    for xyIndex=1:size(globalXCoords,1);
        x=globalXCoords(xyIndex);
        y=globalYCoords(xyIndex);
        ...
        if norm(gradient) > gradientNormThreshold
